...
def allwise_image(ra, dec, radius): 
    ...
    w3_list = location_data['w3mpro'].tolist()
    w4_list = location_data['w4mpro'].tolist()
    ...
    plt.title('AllWISE Search', fontdict = fontdict_1)
    ...
    if len(location) == 0: 
        w1, w2, w3, w4 = np.nan, np.nan, np.nan, np.nan
        plt.close('all')
        plt.figure().clear()
        return w1, w2, w3, w4
    ...
    if str(location[2]) == 'Axes(0.04,0.012;0.92x0.04)': 
        ...
        w3 = w3_list[list_location]
        w4 = w4_list[list_location]
        return w1, w2, w3, w4
...
def allwise_table(ra, dec, radius): 
    ...
    location_url = 'https://irsa.ipac.caltech.edu/cgi-bin/Gator/nph-query?catalog=allwise_p3as_psd&spatial=cone&radius=' + str(radius/2) + '&radunits=arcsec&objstr=' + str(ra_tot) + '+' + str(dec_tot) + '&size=' + str(radius/2) + '&outfmt=1&selcols=ra,dec,w1mpro,w2mpro,w3mpro,w4mpro'
