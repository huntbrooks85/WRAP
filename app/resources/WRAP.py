...
def wrap(ra, dec, radius, catalog_list):
    ...
    for q in range(len(catalog_info)): 
        if catalog_info[q]['name'] in catalog_list: 
            table = table_query(ra, dec, radius, catalog_info[q]["table_id"])
            images, w = image_query(ra, dec, radius, catalog_info[q]["image_id"])
            click_data = image_plot(ra, dec, radius, catalog_info[q], table, images, w)
    ...
    df = pd.DataFrame(photometry, columns=photometry_name)
    return df
...
if __name__ == "__main__":
    ...
    parser.add_argument("catalogs", nargs="+")  # Accepts multiple catalogs
